...
{
    public class ItemPauldronLeft : DaggerfallUnityItem
    {
        public const int templateIndex = 1303;

        public ItemPauldronLeft() : base(ItemGroups.Armor, templateIndex)
        {
        }
                ...
                if (VanillaArmorReplacer.Instance.textureArchive == 3)
                {
                    ...
                    if (NativeMaterialValue == VanillaArmorReplacer.Instance.GetLeatherMaterialValue(nativeMaterialValue))
                    {
                        if (!HasCustomEnchantments && !HasLegacyEnchantments)
                            shortName = "Left " + VanillaArmorReplacer.Instance.namePauldronLeather;
                    }
                    else if (NativeMaterialValue == VanillaArmorReplacer.Instance.GetChainMaterialValue(nativeMaterialValue))
                    {
                        if (!HasCustomEnchantments && !HasLegacyEnchantments)
                            shortName = "Left " + VanillaArmorReplacer.Instance.namePauldronChain;
                    }
                    else
                    {
                        if (!HasCustomEnchantments && !HasLegacyEnchantments)
                            shortName = "Left " + VanillaArmorReplacer.Instance.namePauldronPlate;
                    }
        ...
        public override ItemData_v1 GetSaveData()
        {
            ...
            data.className = typeof(ItemPauldronLeft).ToString();
            return data;
        }
