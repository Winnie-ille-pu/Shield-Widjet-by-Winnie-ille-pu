...
namespace FastTravelEncounters
{
    ...
    public class FoeSpawner : MonoBehaviour
    {
        ...
        public MobileTypes[] lastFoeTypes;
        ...
        void Update()
        {
            ...
            if (FoeTypes != null && FoeTypes != lastFoeTypes && FoeTypes.Length > 0)
            {
                DestroyOldFoeGameObjects(pendingFoeGameObjects);
                SetFoeGameObjects();
                lastFoeTypes = FoeTypes;
            }
        ...
        /// </summary>
        public void SetFoeGameObjects(Transform parent = null)
        {
            List<GameObject> gameObjectsList = new List<GameObject>();

            foreach (MobileTypes mobile in FoeTypes)
            {
                GameObject[] go = GameObjectHelper.CreateFoeGameObjects(GameManager.Instance.PlayerObject.transform.position, mobile, 1, alliedToPlayer: AlliedToPlayer);
                gameObjectsList.AddRange(go);
            }

            GameObject[] gameObjects = gameObjectsList.ToArray();
        ...
        void PlaceFoeFreely(GameObject[] gameObjects, float minDistance = 5f, float maxDistance = 20f)
        {
            ...
            Ray ray = new Ray(currentOrigin, spawnDirection);
            ...
            else
            {
                ...
                currentPoint = currentOrigin + spawnDirection * UnityEngine.Random.Range(minDistance, maxDistance);
            }
