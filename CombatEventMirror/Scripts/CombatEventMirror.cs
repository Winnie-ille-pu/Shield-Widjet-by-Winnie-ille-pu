...
public class CombatEventMirror : MonoBehaviour
{
    static Mod mod;
    ...
    public static void Init(InitParams initParams)
    {
        ...
        go.AddComponent<CombatEventMirror>();
    }

    public static CombatEventMirror Instance;
    ...
    void Awake()
    {
        ...
        //Run mirroring code when game is started or loaded to make sure all mods that add to VCEH events have already done so
    ...
    void ModCompatibilityChecking()
    {
        ...
        if (ceh != null)
        {
            ...
            ModManager.Instance.SendModMessage(ceh.Title, "getSavingThrowEvent", null, (string message, object data) =>
            {
                OnSavingThrow = (Action<DFCareer.Elements, DFCareer.EffectFlags, DaggerfallEntity, int>)data;
            }
            );
        }
    }
    ...
    public static int CalculateAttackDamage(DaggerfallEntity attacker, DaggerfallEntity target, bool isEnemyFacingAwayFromPlayer, int weaponAnimTime, DaggerfallUnityItem weapon)
    {
        ...
        DaggerfallUI.Instance.PopupMessage("WE ARE RUNNING THE MIRROR'S COMBAT FORMULA");
