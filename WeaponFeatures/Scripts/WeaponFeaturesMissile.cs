...
public class WeaponFeaturesMissile : MonoBehaviour
{
    ...
    public uint modelID;
    public DaggerfallMissile dfMissile;
    LayerMask layerMask;
    ...
    private void Start()
    {
        ...
        if (isArrow)
        {
            ...
            goModel = GameObjectHelper.CreateDaggerfallMeshGameObject(modelID, transform);

            //GameObject lightSource = GameObjectHelper.InstantiatePrefab(DaggerfallUnity.Instance.Option_InteriorLightPrefab.gameObject, string.Empty, goModel.transform, goModel.transform.position);
    ...
    void DoStep()
    {
        Vector3 step = (direction * MovementSpeed) * Time.fixedDeltaTime;

        if (gravityMod > 0)
        {
            gravityCurrent += gravity * gravityMod * Time.fixedDeltaTime;
            step += Vector3.down * gravityCurrent;
        }

        Ray ray = new Ray(transform.position, step.normalized);
        RaycastHit[] hits = Physics.SphereCastAll(transform.position, ColliderRadius, step.normalized, step.magnitude, layerMask);

        if (hits.Length > 0)
        {
            foreach (RaycastHit hittee in hits)
            {
                if (hittee.collider.gameObject.GetComponent<DaggerfallEntityBehaviour>() != null)
                {
                    DoCollision(hittee);
                    return;
                }
        ...
        if (Physics.Raycast(ray, out hit, step.magnitude, layerMask))
        {
            DoCollision(hit);
            return;
        }
    ...
    void DoCollision(RaycastHit hit)
    {
        ...
        entityBehaviour = hit.collider.transform.GetComponent<DaggerfallEntityBehaviour>();
        ...
        if (isArrow)
        {
            AssignBowDamageToTarget(hit.collider);

            WeaponFeatures.Instance.RaiseOnMissileHitEvent(gameObject, hit.collider.gameObject, hit.point);
